...
bool isWindowActive()
{
	return FindWindowW(NULL, L"IV:MP T4") != NULL;
}
...
public:
	...
	int Calculate(const char* f)
	{
		...
		// Open the file
		FILE * fFile;
		if(fopen_s(&fFile, f, "rb") != 0)
		{
			return 3;
		}

		// Read file data in 512 byte blocks
...
unsigned int checkSum(const char* file)
{
...
bool filesCheck(bool isGameOpen)
{
	struct tFile
	{
		char* path;
		...
		tFile(char* path, unsigned int sum, bool checkIfGameOpen)
		{
			this->path = path;
			this->sum = sum;
			this->gameOpen = checkIfGameOpen;
		}
	...
	bool t_eflc = false;
		...
		#define SAVEGAME "comMP//" GAME "//SGTA414"
		t_eflc = true;
	#else
		#define GAME "GTA4"
		#define MYARRAYSIZE 8
		#define SAVEGAME "comMP//" GAME "//SGTA412"
	#endif

	LINFO << "Luancher: " << (int)t_eflc;

	tFile files[MYARRAYSIZE] = {
		tFile("comMP//bin//ClientLauncher.exe", t_eflc ? 4180782455 : 2118686630, false),
		tFile("comMP//ClientManager.dll", 113520167, false),
		tFile("comMP//Client.dll", 3843026543, true),
		tFile("comMP//config//anims.dat", 2084087521, true),
		tFile(SAVEGAME, t_eflc ? 3174405877 : 3404312055, true),
		tFile("comMP//" GAME "//ControlMap.dat", 0, true),
		tFile("clientLibs.dll", 2739823661, false),
		tFile("bass.dll", 1828190289, false),
		#if EFLC
			tFile("common/data/ivmpdef.dat", 0, true),
			tFile("common/data/ivmpvehicles.ide", 0, true),
			tFile("TBoGT/ivmpcontent.dat", 0, true),
			tFile("TBoGT/IVstup.xml", 0, true),
			tFile("TLAD/IVstup.xml", 0, true),
		#endif
	};
	...
	{
		if(isGameOpen && !files[i].gameOpen)
		{
			continue;
		}

		unsigned int sum = checkSum(files[i].path);
		if(sum == 0 || (sum != files[i].sum && files[i].sum != 0))
		{
			LINFO << files[i].path << ": " << sum;
			//returnB = false;
		}
	}
	return returnB;
...
bool checkEFLCVersion(std::string& buf)
{
	DWORD verHandle= 0;
	UINT size = 0;
	LPBYTE lpBuffer = NULL;
	DWORD verSize = GetFileVersionInfoSize(GAMENAME ".exe", &verHandle);
	std::string s;

	if(verSize != NULL)
	{
		LPSTR verData = new char[verSize];
		if(GetFileVersionInfo(GAMENAME ".exe", verHandle, verSize, verData))
		{
	...
	if(s.empty())
	{
		LINFO << "Error: unable to check executable version";
	}

	#if GTAIV
		//LINFO << "IV";
		if(s != std::string("1.0.7.0") && s != std::string("1.0.8.0")) return false;
	#else
		//LINFO << "LC";
		//if(s != std::string("1.1.2.0") && s != std::string("1.1.3.0") && s != "1.2.0.32") return false;
	#endif
