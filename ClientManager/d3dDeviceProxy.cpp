...
LONG firstResolution[2];
char FINDWINDOW[8] = "EFLC";

bool d3dHook::setWindowed()
{
	if(execution::getPatch() < 0x1120) sprintf_s(FINDWINDOW, "%s", "GTAIV");
	...
	if(infile.is_open())
	{
		...
		{
			if(line == std::string("windowed=1"))
			{
				LINFO << "Windows mode enabled";
				windowed = true;
			}
		}
	}
...
HRESULT __stdcall IDirect3DDevice9Proxy::Present ( CONST RECT* pSourceRect, CONST RECT* pDestRect, HWND hDestWindowOverride, CONST RGNDATA* pDirtyRegion)
{
	...
	{
		LINFO << "D3D Device lost";
		d3d::callOnLost(m_pDevice);
		deviceLost = true;
	}
	return hr;
}
...
HRESULT __stdcall IDirect3DDevice9Proxy::Reset(D3DPRESENT_PARAMETERS* pPresentationParameters)
{
	...
	LINFO << "D3D Reset Device: " << (int)deviceLost;
	d3d::callOnLost(m_pDevice);

	if(firstResolution[0] == 0)
	{
		firstResolution[0] = pPresentationParameters->BackBufferWidth;
		firstResolution[1] = pPresentationParameters->BackBufferHeight;
	}
	...
	if(SUCCEEDED(hr))
	{
		d3d::callOnReset(m_pDevice, pPresentationParameters);
		deviceLost = false;

		if(!windowNameSet)
		{
			HWND cw = FindWindow(NULL, FINDWINDOW);
			//LINFO << "FINDWINDOW: " << FINDWINDOW;
			if(cw != NULL)
			{
				LINFO << "nFound";
				SetWindowTextA(cw, "IV:MP T4");
				SetWindowLong(cw, GWL_STYLE, GetWindowLong(cw, GWL_STYLE) & ~WS_MINIMIZEBOX & ~WS_MAXIMIZEBOX);

				cw = FindWindowW(NULL, L"IV:MP T4");
				if(cw != NULL)
				{
					windowNameSet = true;
					if(!windowed)
		...
		if(windowed)
		{
			...
			RECT r;
			GetClientRect(pPresentationParameters->hDeviceWindow, &r);
			UINT x = pPresentationParameters->BackBufferWidth + (pPresentationParameters->BackBufferWidth - r.right);
			UINT y = pPresentationParameters->BackBufferHeight + (pPresentationParameters->BackBufferHeight - r.bottom);

			SetWindowPos(pPresentationParameters->hDeviceWindow, HWND_NOTOPMOST, 0, 0, x, y, SWP_FRAMECHANGED | SWP_SHOWWINDOW | SWP_NOMOVE);
		}
	}
	else
	{
		LINFO << "D3D Reset failed: " << GetLastError() << ", " << DXGetErrorString(hr) << ", " << DXGetErrorDescription(hr);
	}
	return hr;
}
