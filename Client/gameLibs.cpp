...
unsigned int gameLibs::crc(const char* path)
{
	...
	CFileChecksum cf;
	//LINFO << paramHelper::DwordToString(list, size).c_str(); //remove easy log from inclues
	if(cf.Calculate(path) != 1)
	{
		LINFO << "File didnt open";
		return 2;
	}
	return cf.GetChecksum();
}
void gameLibs::checkSum(RakNet::BitStream& bsIn)
{
	RakNet::RakString s;
	bsIn.Read(s);

	RakNet::BitStream rs;
	rs.Write((MessageID)IVMP);
	rs.Write(RUN_CHECKSUM);

	rs.Write(crc(s.C_String()));
	rs.Write(s);
	networkManager::sendBitStream(rs);
	//LINFO << "Sent";
}

void gameLibs::moduleCheck(RakNet::BitStream& bsIn)
{
	//char wnd_title[256];
	HWND windowHandle = GetForegroundWindow();
	//GetWindowText(hwnd,wnd_title,sizeof(wnd_title));	
	
	//HWND windowHandle = FindWindowW(NULL, L"IV Multiplayer 0.2");
	DWORD processID;
	GetWindowThreadProcessId(windowHandle, &processID);
	//LINFO << "Process id: " << processID;
    ...
    if(NULL != hProcess)
	{
		...
		if(EnumProcessModules(hProcess, hMods, sizeof(hMods), &cbNeeded))
		{
			for(unsigned int i = 0; i < (cbNeeded / sizeof(HMODULE)); i++)
			{
				...
				{
					//LINFO << szModName << ", " << hMods[i];
					std::string s = szModName;
					size_t size = s.size();
					if(s.substr(size - 14, size) == std::string("ScriptHook.dll"))
					{
						scriptHook = true;
					}
				}
		...
		RakNet::BitStream rs;
		rs.Write((MessageID)IVMP);
		rs.Write(RUN_HACK_CHECK);
		rs.Write((int)scriptHook);
